...
public class AddSectionToShowUseCaseTest extends UseCaseTest {
        ...
        verify(showGateway, times(1))
                .update(argThat(saved -> saved.getId() != null
                        ...
                        && saved.getSections().stream()
                                .anyMatch(
                                        section -> section.getName().getValue().equals("General")
    ...
    public void givenLargeSection_whenExecute_shouldPersistShellAndPublishEvent() {
        final var entity = entity();
        final var command =
                AddSectionToShowCommand.with(entity.getId().getValue(), "Arena", "Description", 1500, PRICE);
        when(showGateway.findById(entity.getId())).thenReturn(Optional.of(entity));
        when(showGateway.update(any())).thenAnswer(returnsFirstArg());
        ...
        assertEquals(entity.getId().getValue(), output.id());
        verify(showGateway, times(1))
                .update(argThat(saved -> saved.getSections().size() == 2
                        && saved.getTotalSpots() == 1502
                        && saved.getSections().stream()
                                .filter(section -> section.getName().getValue().equals("Arena"))
                                .allMatch(section -> section.getSpots().isEmpty() && section.getTotalSpots() == 1500)));
        verify(eventPublisher, times(1))
                .publish(argThat(event -> event instanceof SpotsGenerationRequested requested
        ...
        verify(showGateway, times(1))
                .appendSpots(
                        ...
                        argThat(spots -> spots.size() == 1500
                                && spots.stream()
                                        .allMatch(spot ->
                                                spot.getLocation().getValue().matches("B\\d{5}"))));
    }
