...
@DisplayName("Change show description use case")
class ChangeShowDescriptionUseCaseTest {
    ...
    private final DefaultChangeShowDescriptionUseCase useCase = new DefaultChangeShowDescriptionUseCase(gateway);
    ...
    void changesOnlyRequestedFieldAndPersists() {
        ...
        final var command = ChangeShowDescriptionCommand.with(id, value);
    ...
    void rejectsInvalidValueWithoutMutationOrPersistence() {
        ...
        final var updatedAt = entity.getUpdatedAt();
        when(gateway.findById(entity.getId())).thenReturn(Optional.of(entity));
        final var command = ChangeShowDescriptionCommand.with(id, "x".repeat(1001));
