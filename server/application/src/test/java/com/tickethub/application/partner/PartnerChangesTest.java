...
class PartnerChangesTest {
    ...
    private final Address newAddress =
            Address.create("Rua B", "20", "Sala 1", "Centro", "Sao Paulo", "SP", "Brasil", "01001001");
    private final Partner partner =
            Partner.create("Cinema Nova", "11222333000181", address, VALID_EMAIL, PASSWORD_HASH);
    ...
    void requiresAddressOnCreation() {
        ...
        assertEquals(
                "'address' should not be null",
                result.getLeft().firstError().message(),
    ...
    void persistsAddressOnCreationAndExposesItInQueries() {
        ...
        new DefaultCreatePartnerUseCase(gateway, passwordHasher)
                .execute(CreatePartnerCommand.with("Cinema Nova", "11222333000181", address, VALID_EMAIL, RAW_PASSWORD))
                .getRight();
        verify(gateway).create(argThat(p -> address.equals(p.getAddress())));
        assertEquals(
                address,
                GetPartnerOutput.from(partner).address(),
                () -> "Get partner output should expose the persisted address");
        assertEquals(
                address,
                ListPartnersOutput.from(partner).address(),
    ...
    void changesOnlyAddress() {
        ...
        assertEquals(
                id,
                new DefaultChangePartnerAddressUseCase(gateway)
                        .execute(ChangePartnerAddressCommand.with(id, newAddress))
                        .getRight()
                        .id(),
        ...
        assertFalse(
                partner.getUpdatedAt().isBefore(updatedAt),
    ...
    void rejectsInvalidAddressWithoutMutationOrPersistence() {
        ...
        assertEquals(
                "'address' should not be null",
                new DefaultChangePartnerAddressUseCase(gateway)
                        .execute(ChangePartnerAddressCommand.with(id, null))
                        .getLeft()
                        .firstError()
                        .message(),
