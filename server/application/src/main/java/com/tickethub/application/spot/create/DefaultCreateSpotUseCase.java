...
public class DefaultCreateSpotUseCase extends CreateSpotUseCase {
    ...
    public DefaultCreateSpotUseCase(
            final SpotGateway spotGateway, final SectionGateway sectionGateway, final int seatNumberWidth) {
    ...
    public Either<Notification, CreateSpotOutput> execute(final CreateSpotCommand command) {
        try {
            ...
            final Spot entity = Optional.ofNullable(command.location())
                    .map(Spot::create)
