...
public interface OrderAPI {
    ...
    @Operation(
            ...
            description =
                    "Opens a PENDING order and atomically reserves each spot for 15 minutes; returns the order with its expiry. "
                            + "Send Idempotency-Key to make retries safe: the same key returns the original order instead of reserving twice")
    @ApiResponses({
        @ApiResponse(
                responseCode = "201",
                description =
                        "Order opened (or replayed for a known Idempotency-Key); returns the order and its URL in the Location header"),
        @ApiResponse(
                responseCode = "400",
                description =
        ...
        @ApiResponse(
                responseCode = "404",
                description = "The customer or one of the spots was not found for the supplied identifiers"),
        @ApiResponse(
                responseCode = "422",
                description = "One of the spots is unpublished, already reserved or used, or has no price"),
        @ApiResponse(
                responseCode = "500",
    ...
    @Operation(
            summary = "Pay Order",
            description =
                    "Creates a provider charge for a PENDING order; retrying an already charged order returns the current charge")
    @ApiResponses({
    ...
    @Operation(
            summary = "Get Order",
            description =
                    "Returns an order with its status, charge and reserved spots; use it to poll payment confirmation")
    @ApiResponses({
