...
public class UseCaseMonitoringAspect {
    ...
    public Object monitor(final ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        final String inputSummary = String.valueOf(joinPoint.getArgs());
        ...
        try {
            ...
            audit(action, inputSummary, startedAt, AuditOutcome.INFRA_ERROR, String.valueOf(t.getMessage()));
            throw t;
        }
        if (result instanceof Either<?, ?> either && either.isLeft()) {
            final var notification = ((Either<Notification, ?>) either).getLeft();
            audit(action, inputSummary, startedAt, outcomeOf(notification), firstMessage(notification));
                ...
                && optional.orElse(null) instanceof Notification notification) {
            audit(action, inputSummary, startedAt, outcomeOf(notification), firstMessage(notification));
        } else {
    ...
    private void audit(final String action, final String input, final long startedAt,
