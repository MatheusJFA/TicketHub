...
public class UseCaseMonitoringAspect {
    ...
    public Object monitor(final ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        try {
            ...
            audit(action, inputSummary, startedAt, AuditOutcome.UNAVAILABLE,
                    Optional.ofNullable(status.getReason()));
            throw status;
        } catch (final Throwable t) {
            audit(action, inputSummary, startedAt, AuditOutcome.INFRASTRUCTURE_ERROR,
                    Optional.ofNullable(t.getMessage()));
            throw t;
        }
        failureOf(result).ifPresentOrElse(
                notification -> audit(action, inputSummary, startedAt, outcomeOf(notification),
                        firstMessage(notification)),
                () -> audit(action, inputSummary, startedAt, AuditOutcome.SUCCESS,
                        Optional.empty()));
        return result;
    }
    ...
    private void audit(final String action, final Optional<String> input, final long startedAt,
        ...
        final var entry = new AuditEntry(Instant.now(),
                ...
                action, input.orElse(""), outcome, error.orElse(null), durationMs);
