    ...
    MongoClientSettingsBuilderCustomizer mongoTimeouts(final MongoConnectionProperties properties) {
        ...
        return builder -> builder
                ...
                .applyToClusterSettings(cluster -> cluster
                        .serverSelectionTimeout(selectionMillis, TimeUnit.MILLISECONDS));
    }

    private static int positiveMillis(final Duration duration) {
        final var millis = duration.toMillis();
        Assert.isTrue(millis > 0 && millis <= Integer.MAX_VALUE, "MongoDB timeout must be a positive number of milliseconds within integer range");
        return (int) millis;
    }
}
