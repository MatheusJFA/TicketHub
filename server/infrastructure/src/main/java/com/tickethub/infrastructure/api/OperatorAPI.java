...
public interface OperatorAPI {
    ...
    @Operation(summary = "Create Operator (master only)")
    @ApiResponses({
        @ApiResponse(
                responseCode = "201",
                description =
                        "Operator created successfully; returns its identifier and the resource URL in the Location header"),
        @ApiResponse(
                responseCode = "400",
                description =
    ...
    ResponseEntity<IdResponse> createOperator(@RequestBody CreateOperatorRequest input);
}
