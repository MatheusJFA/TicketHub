...
public class SpotGenerationListener {
    ...
    public SpotGenerationListener(
            final GenerateSectionSpotsUseCase generateSectionSpots, final ObjectMapper objectMapper) {
        this.generateSectionSpots = requireNonNull(generateSectionSpots, "'generateSectionSpots' should not be null");
    ...
    public void onMessage(final String payload) {
        ...
        final var result = generateSectionSpots.execute(
                new GenerateSectionSpotsCommand(message.showId(), message.sectionId(), message.sectionCode()));
        if (result.isLeft()) {
            final var notification = result.getLeft();
            final var detail = Optional.ofNullable(notification.firstError())
                    .map(first -> first.message())
                    .orElse("unknown");
            throw new SpotGenerationException("Spot generation failed: " + detail, notification.getCause());
        }
    }
}
