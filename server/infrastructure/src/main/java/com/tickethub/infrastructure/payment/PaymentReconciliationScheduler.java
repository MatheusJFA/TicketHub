...
public class PaymentReconciliationScheduler {
    ...
    public PaymentReconciliationScheduler(
            final ReconcileOrdersUseCase reconcileOrders, final ObjectProvider<CacheManager> cacheManager) {
    ...
    public void reconcile() {
        ...
        LOG.info(
                "Payment reconciliation settled={} refunded={} expired={}",
                output.settled().size(),
                output.refunded().size(),
                output.expired().size());
        Optional.ofNullable(cacheManager.getIfAvailable()).ifPresent(manager -> {
            for (final var name : new String[] {
                TickethubCacheProperties.SHOWS, TickethubCacheProperties.SECTIONS, TickethubCacheProperties.SPOTS
            }) {
