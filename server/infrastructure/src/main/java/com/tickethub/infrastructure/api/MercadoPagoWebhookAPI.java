...
public interface MercadoPagoWebhookAPI {
    ...
    @Operation(summary = "Mercado Pago webhook",
            description = "Real provider callback: verifies the x-signature, then reads the payment status from the MP API (never trusted from the callback). Non-payment topics are acknowledged without effect")
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "Webhook verified and processed, or non-payment topic ignored"),
