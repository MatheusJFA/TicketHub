...
class InfrastructureConfigurationTest {
    ...
    void loadsMongoDatabaseAndBoundedTimeouts() {
        contextRunner
                .withPropertyValues(
                        ...
                        "tickethub.mongo.server-selection-timeout=4s")
                .run(context -> {
                    assertThat(context).hasNotFailed();
                    assertThat(context.getBean(MongoProperties.class).getDatabase())
                            .isEqualTo("config-test");
    ...
    void loadsKafkaProducerConsumerAndListenerSettings() {
        contextRunner.run(context -> {
            ...
            assertThat(kafka.getConsumer().getValueDeserializer().getSimpleName())
                    .isEqualTo("StringDeserializer");
    ...
    void usesSameConfiguredTopicForAdminAndTemplate() {
        contextRunner
                .withPropertyValues(
                        ...
                        "spring.kafka.consumer.group-id=custom-group")
                .run(context -> {
                    assertThat(context).hasNotFailed();
    ...
    void rejectsUnboundedMongoTimeout() {
        contextRunner
                .withPropertyValues("tickethub.mongo.read-timeout=0s")
    ...
    void rejectsInvalidTopicPartitionCount() {
        contextRunner
                .withPropertyValues("tickethub.kafka.topic.partitions=0")
                .run(context -> assertThat(context).hasFailed());
    }
}
