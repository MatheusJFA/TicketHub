...
class MercadoPagoWebhookHandlerTest {
    ...
    private record Fixture(MercadoPagoWebhookHandler handler, PaymentGateway gateway,
    ...
    private Charge paidCharge() {
        final var charge = Charge.create(ChargeID.from(DATA_ID), OrderID.from("order-1"),
    ...
    void givenVerifiedPayment_whenHandle_thenConfirmsWithApiStatus() {
        ...
        when(fixture.useCase().execute(any())).thenReturn(
                Either.right(new ConfirmPaymentOutput("order-1", "PAID")));

        final var result = fixture.handler()
                .handle("ts=" + TS + ",v1=" + V1, REQUEST_ID, DATA_ID, "payment");

        assertThat(result).isPresent();
        assertThat(result.get().isRight()).isTrue();
        verify(fixture.useCase()).execute(argThat(command ->
    ...
    void givenBadSignature_whenHandle_thenRejectsBeforeFetching() {
        ...
        assertThrows(InvalidWebhookSignatureException.class, () -> fixture.handler()
                .handle("ts=" + TS + ",v1=" + "0".repeat(64), REQUEST_ID, DATA_ID, "payment"));
    ...
    void givenUnknownPayment_whenHandle_thenReturnsNotFound() {
        ...
        assertThat(result.get().getLeft().getErrors().get(0).message())
                .isEqualTo("Charge not found: " + DATA_ID);
    }
