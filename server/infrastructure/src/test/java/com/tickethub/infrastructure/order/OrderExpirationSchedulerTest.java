...
class OrderExpirationSchedulerTest {
    ...
    private record Fixture(OrderExpirationScheduler scheduler, ExpireOrdersUseCase useCase, Cache spots) {}
    ...
    void givenExpiredOrders_whenSweep_thenClearsSpotsCache() {
        ...
        when(fixture.useCase().execute()).thenReturn(Either.right(ExpireOrdersOutput.from(List.of("order-1"))));
    ...
    void givenNothingExpired_whenSweep_thenKeepsCache() {
        final var fixture = fixture();
        when(fixture.useCase().execute()).thenReturn(Either.right(ExpireOrdersOutput.from(List.of())));
