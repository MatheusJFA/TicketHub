...
class ShowControllerTest {
    ...
    void givenAValidCommand_whenCallsCreateShow_shouldReturnShowId() throws Exception {
        ...
        mvc.perform(post("/shows")
                        .header("Authorization", bearer("show:create"))
    ...
    void givenOwner_whenCallsUpdateShow_thenSucceeds() throws Exception {
        ...
        when(updateShow.execute(any())).thenReturn(Either.right(new UpdateShowOutput("show-1")));

        mvc.perform(put("/shows/show-1")
                        .header("Authorization", bearer("show:write"))
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
    ...
    void givenSections_whenListsShowSections_thenReturnsPage() throws Exception {
        when(listShowSections.execute(any())).thenReturn(Either.right(new Pagination<>(0, 10, 0, java.util.List.of())));
